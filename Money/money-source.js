...
(function () {
    ...
    Money.prototype = {
        toString: function (amountFormat, symbolFormat) {
        ...
        writeAmount: function (amount, decimalCount, shortFormat) {
        ...
        roundAmount: function (amount, digitCount) {
            var precision = Math.pow(10, digitCount);
            return Math.round(amount * precision) / precision;
        }
    }

    Money.setCulture = function (name) {
        var culture = Money.Cultures[name != null ? name.replace('-', '') : 'enUS']
        if (culture == null)
    ...
    //#region Money.CurrencyPositiveFormats

    Money.CurrencyPositiveFormats = ["$n", "n$", "$ n", "n $"];

    //#endregion

    //#region Money.CurrencyNegativeFormats

    Money.CurrencyNegativeFormats = ["($n)", "-$n", "$-n", "$n-", "(n$)", "-n$", "n-$", "n$-", "-n $", "-$ n", "n $-", "$ n-", "$ -n", "n- $", "($ n)", "(n $)"];

    //#endregion

    //#region Money.Currencies
    ...
    // Culture @class
    function Culture(isoCode, symbol, negativeSign, positiveFormat, negativeFormat, decimalSeparator, groupSeparator, zero, groupSizes) {
    ...
    function valueAt(a, i, d) {
        return a.length > i ? a[i] : d;
    }
